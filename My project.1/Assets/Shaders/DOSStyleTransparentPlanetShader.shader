Shader "Custom/DOSStyleTransparentPlanetShader"
{
    Properties
    {
        _WaterColor ("Water Color", Color) = (0.1, 0.3, 0.6, 1.0)
        _LandColor ("Land Color", Color) = (0.3, 0.5, 0.2, 1.0)
        _AtmosphereColor ("Atmosphere Color", Color) = (0.7, 0.8, 1.0, 0.5)
        _AtmosphereSize ("Atmosphere Size", Range(0.0, 1.0)) = 0.3
        _AtmosphereIntensity ("Atmosphere Intensity", Range(0.0, 1.0)) = 0.6
        [Space(10)]
        
        [Header(Transparency Settings)]
        _PlanetOpacity ("Planet Opacity", Range(0.0, 1.0)) = 1.0
        _AtmosphereOpacity ("Atmosphere Opacity", Range(0.0, 1.0)) = 0.5
        _EdgeFade ("Edge Fade", Range(0.0, 5.0)) = 1.0
        [Space(10)]
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (appdata v)
            {
                ...
                // Save the position for fresnel calculations
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                // Blend planet and atmosphere with transparency
                float4 finalColor;
                finalColor.rgb = lerp(planetColor.rgb, _AtmosphereColor.rgb, rim);
                
                // Quantize the color to reduce color depth (DOS style)
                finalColor.rgb = QuantizeColor(finalColor.rgb, _ColorLevels);
                
                // Calculate opacity
                float atmosphereAlpha = rim * _AtmosphereOpacity;
                float planetAlpha = _PlanetOpacity * (1.0 - rim) + atmosphereAlpha;
                
                // Apply edge fade for smoother blending with skybox
                float edgeFade = 1.0 - pow(rim, _EdgeFade);
                
                // Set the final alpha
                finalColor.a = planetAlpha * edgeFade;
